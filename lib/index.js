 ...
 */
const urlTolinkParams = (url, method) => {
...
`;
                    ...
                    const cliTypeString = folder.cliType === 'vite' ? 'import.meta.env.VITE_APP_HOST' : 'process.env.VUE_APP_HOST';
...
export function ${method.toLowerCase() + url2name(api.url)}(${method.toUpperCase() === 'GET' ? 'params' : 'data'}, options) {
  return ${improtAxiosPath ? 'request' : 'window.axios'}({
    url: \`\${baseURL}${urlTolinkParams(api.url, method)}\`,${includeBaseURL !== false
                            ? `
