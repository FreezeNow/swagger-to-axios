...
`;
                ...
                if (improtAxiosPath) {
                    fileContent += `import ${!namedImport ? 'request' : namedImport === 'request' ? `{ request }` : `{ ${namedImport} as request }`} from '${improtAxiosPath}';
`;
                }
                fileContent += `
...
`;
  ...
  `;
                    fileContent += `return ${improtAxiosPath ? `request` : 'window.axios'}${typeScript ? `<${namespace}.${functionName}Response>` : ''}(`;
                    if (!urlInOptions) {
