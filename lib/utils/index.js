...
export const getSwaggerJson = async ({ url, }) => {
    try {
        ...
        if ('swagger' in swagger) {
            ...
            const openapi = await SwaggerParser.dereference(result.openapi);
        ...
        if (paths.hasOwnProperty(path)) {
            ...
            for (const name in methods) {
                if (methods.hasOwnProperty(name)) {
                    ...
                    const { type: paramType, required: paramRequired } = parametersToTypeScript(method?.parameters, method?.requestBody, name);
                    tag.apiList.push({
                        url: path,
                        method: name,
                        comment: {
...
/** 将参数列表转换为 TS 类型 */
export const parametersToTypeScript = (parameters = [], requestBody, method) => {
    const fields = [];
...
/** 链接变成名称 */
export const urlToName = (str) => {
    const resolved = str.replace(/\{([^}]+)\}/g, (_match, name) => name
        .split(/[\/\-_]/)
        .reduce((acc, p) => acc + upperFirstCase(p), ''));
    return resolved
        .split(/[\/\-_]/)
        .reduce((acc, p) => upperFirstCase(acc) + upperFirstCase(p));
};
/**
