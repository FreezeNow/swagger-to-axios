...
// 链接变成名称
const url2name = (str) => str.split('/').reduce((accumulator, currentValue) => ucFirst(accumulator) + ucFirst(currentValue));
/** 创建所有 API 文件
    ...
    try {
        ...
        for (const folder of swagger.list) {
            ...
            for (const file of folder.list) {
                let fileContent = `// ${file.comment}
const baseURL = '${folder.baseURL}';
const host = \`${folder.host.includes('127.0.0.1')
                    ? folder.host
                    : "${process.env.VUE_APP_HOST ? process.env.VUE_APP_HOST : '183.134.197.66:20002'}"}\`;
`;
                ...
                for (const api of apiList) {
                    for (let l = 0; l < api.method.length; l++) {
                        ...
                        fileContent += `
//  ${api.comment[l]}
${improtAxiosPath ? `import request from ${improtAxiosPath}` : ''}
export function ${method.toLowerCase() + url2name(api.url)}(${method.toUpperCase() === 'GET' ? 'params' : 'data'}, options) {
  return ${improtAxiosPath ? 'request' : 'window.axios'}({
    url: \`\${baseURL}${api.url}\`,${includeBaseURL !== false
                            ? `
    baseURL: \`http://\${host}\`,`
                            : ''}
    method: '${method}',
